...
void ChatServer::AcceptAll(int& fd)
{
  while (true)
  {
    ...
    if (getpeername(cs, reinterpret_cast<sockaddr*>(&ss), &slen) == 0)
    {
...
void ChatServer::ModClientWritable(int fd)
{
  ...
  if (it->second && it->second->IsWantSend())
  {
    mask |= EPOLLOUT;
  }
